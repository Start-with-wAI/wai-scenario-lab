...
def get_public_scenario_lab_config() -> dict:
    """Retrieves selected config items for the Scenario Lab.

    Returns:
        A dictionary containing app metadata, shared UI copy, and all scenarios configuration.

    Raises:
        ConfigLoadError: If required keys are missing.
    """
...
def render_question_html(q: dict, value: any = None, error: str = None) -> str:
    ...
    if q_type == "text":
        ...
        for opt in options:
            opt_val = opt.get("value", "")
            opt_label = opt.get("label", "")
            selected = "selected" if val_str == opt_val else ""
            options_html += f'<option value="{opt_val}" {selected}>{opt_label}</option>'
        return f"""
        <div class="form-group {has_error_class}">
            <label for="{q_id}">{label}</label>
            {help_html}
            {error_html}
            <select id="{q_id}" name="{q_id}" {required} {aria_describedby}>
                {options_html}
            </select>
        </div>
        """
...
def render_scenario_lab_page(config: dict) -> str:
    """Renders the complete dynamic configuration-driven Scenario Lab landing page."""
    app_meta = config["app"]
...
<head>
    ...
    <style>
        ...
        /* Results / Scenario Brief Rendering */
        .result-section {{
            display: none;
            width: 100%;
        }}
...
<body>
    <div class="container">
        ...
        <header class="header">
            ...
            <span class="badge">Prototype v{app_meta.get("version", "0.2.0")}</span>
        </header>

        <!-- Landing Selection Page -->
        <main id="view-selection">
            <h1>{landing_headline}</h1>
            <p class="lead">{landing_supporting_copy}</p>

            <section class="notice-box">
                <div class="notice-title">Important Disclosures</div>
            ...
            <div class="scenario-grid" id="scenario-cards-container">
                <!-- Dynamically populated from JS -->
            </div>
        </main>

        <!-- Questionnaire Section -->
        <section class="form-section" id="view-form">
            <div class="form-header">
                <div class="form-title" id="lbl-scenario-title">Scenario Title</div>
                <div class="form-context" id="lbl-scenario-context">Context...</div>
            </div>

            <div id="error-summary-container"></div>

            <form id="scenario-questions-form" onsubmit="handleFormSubmit(event)">
                <input type="hidden" id="hidden-scenario-id" name="scenario_id" />
                <div id="dynamic-questions-container">
                    <!-- Dynamically loaded questions go here -->
                </div>

                <div class="form-actions">
                    <button type="button" class="btn btn-secondary" onclick="showSelectionView()">
                        {shared_ui.get("change_scenario_label", "Choose a Different Scenario")}
                    </button>
                    <button type="submit" class="btn btn-primary">
                        {shared_ui.get("question_submit_label", "Analyze My Scenario")}
                    </button>
                </div>
            </form>
        </section>
        ...
        <!-- Processing Screen -->
        <section class="processing-section" id="view-processing">
            <div class="spinner"></div>
            <div class="processing-step" id="lbl-processing-step">Reviewing the information provided</div>
            <div class="processing-sub">Running deterministic wAI safety checks...</div>
        </section>

        <!-- Result View -->
        <section class="result-section" id="view-result">
            <!-- Dynamically populated Scenario Brief -->
            <div id="result-brief-container"></div>

            <div class="result-actions">
                <button type="button" class="btn btn-secondary" onclick="showSelectionView()">
                    {shared_ui.get("change_scenario_label", "Choose a Different Scenario")}
                </button>
                <button type="button" class="btn btn-primary" onclick="restartCurrentScenario()">
                    {shared_ui.get("start_over_label", "Start Over")}
                </button>
            </div>
        </section>
    </div>

    <!-- Configuration Data Injected Here -->
    <script id="scenario-config" type="application/json">
        {config_json_str}
    </script>

    <!-- Client-Side UI Controller -->
    <script>
        const config = JSON.parse(document.getElementById('scenario-config').textContent);
        const scenarios = config.scenarios;
        const sharedUi = config.shared_ui;
        ...
        document.addEventListener('DOMContentLoaded', () => {{
            ...
            Object.keys(scenarios).forEach(key => {{
                ...
                card.onclick = () => selectScenario(s.scenario_id);
                
                card.innerHTML = `
                    <div class="scenario-title">${{escapeHtml(s.title)}}</div>
                    <div class="scenario-desc">${{escapeHtml(s.short_description)}}</div>
                    <div class="scenario-meta">
                        <span>Est. time: ${{escapeHtml(s.estimated_completion_minutes)}} mins</span>
                        <span>Preview: ${{escapeHtml(s.measurement_preview)}} &rarr;</span>
                    </div>
                `;
        ...
        function selectScenario(scenarioId) {{
            ...
            document.getElementById('hidden-scenario-id').value = s.scenario_id;
            document.getElementById('lbl-scenario-title').textContent = s.title;
            document.getElementById('lbl-scenario-context').textContent = s.context;
            ...
            const sortedQuestions = [...s.questions].sort((a, b) => a.order - b.order);
            sortedQuestions.forEach(q => {{
                const group = document.createElement('div');
                group.className = 'form-group';
                group.id = `group-${{q.id}}`;

                let helpHtml = q.help_text ? `<small class="help-text">${{escapeHtml(q.help_text)}}</small>` : '';
                let labelHtml = `<label for="${{q.id}}">${{escapeHtml(q.label)}}</label>`;

                let inputHtml = '';
                if (q.type === 'text') {{
                    inputHtml = `<input type="text" id="${{q.id}}" name="${{q.id}}" ${{q.required ? 'required' : ''}} placeholder="Describe briefly..." />`;
                }} else if (q.type === 'textarea') {{
                    inputHtml = `<textarea id="${{q.id}}" name="${{q.id}}" ${{q.required ? 'required' : ''}} placeholder="Enter details here..."></textarea>`;
                }} else if (q.type === 'number') {{
                    const unitHtml = q.unit ? `<span class="number-unit">${{escapeHtml(q.unit)}}</span>` : '';
                    inputHtml = `
                        <div class="number-input-wrapper">
                            <input type="number" id="${{q.id}}" name="${{q.id}}" ${{q.required ? 'required' : ''}} min="${{q.min ?? 0}}" max="${{q.max ?? 480}}" step="${{q.step ?? 1}}" />
                            ${{unitHtml}}
                        </div>
                    `;
                }} else if (q.type === 'radio') {{
                    labelHtml = `<legend>${{escapeHtml(q.label)}}</legend>`;
                    let optionsHtml = '';
                    q.options.forEach(opt => {{
                        optionsHtml += `
                            <label class="radio-label">
                                <input type="radio" name="${{q.id}}" value="${{escapeHtml(opt.value)}}" ${{q.required ? 'required' : ''}} />
                                <span>${{escapeHtml(opt.label)}}</span>
                            </label>
                        `;
                    }});
                    inputHtml = `<div class="radio-group">${{optionsHtml}}</div>`;
        ...
        function restartCurrentScenario() {{
            const scenarioId = document.getElementById('hidden-scenario-id').value;
            selectScenario(scenarioId);
        }}

        // Dynamic Client-side validation
        function validateForm(scenarioId) {{
            const s = scenarios[scenarioId];
            const errors = {{}};
            let hasErrors = false;
            ...
            s.questions.forEach(q => {{
                let val = '';
                if (q.type === 'radio') {{
                    const selectedOpt = document.querySelector(`input[name="${{q.id}}"]:checked`);
                    val = selectedOpt ? selectedOpt.value : '';
                }} else {{
                ...
                // Required check
                if (q.required && !val) {{
                    errors[q.id] = q.validation?.required ?? 'This field is required.';
                    hasErrors = true;
                    return;
                }}
                ...
                if (val) {{
                    ...
                    if (q.type === 'text' || q.type === 'textarea') {{
                        if (q.max_length && val.length > q.max_length) {{
                            errors[q.id] = q.validation?.too_long ?? `Max length is ${{q.max_length}} characters.`;
                            hasErrors = true;
                        }}
                    }}
                    // Number validation
                    if (q.type === 'number') {{
                        ...
                        if (isNaN(num)) {{
                            errors[q.id] = q.validation?.non_numeric ?? 'Please enter a valid number.';
                            hasErrors = true;
                        }} else {{
                            if (!Number.isInteger(num)) {{
                                errors[q.id] = q.validation?.whole_number ?? 'Please enter a whole number.';
                                hasErrors = true;
                            }}
                            if (q.min !== undefined && num < q.min) {{
                                errors[q.id] = q.validation?.below_min ?? `Minimum value is ${{q.min}}.`;
                                hasErrors = true;
                            }}
                            if (q.max !== undefined && num > q.max) {{
                                errors[q.id] = q.validation?.above_max ?? `Maximum value is ${{q.max}}.`;
                                hasErrors = true;
                            }}
                        }}
                    }}
        ...
        async function handleFormSubmit(event) {{
            ...
            document.querySelectorAll('[id$="-error-msg"]').forEach(el => el.innerHTML = '');
            document.getElementById('error-summary-container').innerHTML = '';

            const {{ hasErrors, errors }} = validateForm(scenarioId);

            if (hasErrors) {{
                // Render Errors
                const summary = document.getElementById('error-summary-container');
                summary.innerHTML = `
                    <div class="error-summary">
                        <div class="error-summary-title">${{escapeHtml(sharedUi.error_messages.form_multiple_errors)}}</div>
                    </div>
                `;
                ...
                Object.keys(errors).forEach(key => {{
                    ...
                    const errMsgDiv = document.getElementById(`${{key}}-error-msg`);
                    if (errMsgDiv) {{
                        errMsgDiv.innerHTML = `<span class="field-error">${{escapeHtml(errors[key])}}</span>`;
                    }}
                }});
                return;
            }}
            ...
            s.questions.forEach(q => {{
                if (q.type === 'radio') {{
                    ...
                    answers[q.id] = val ? parseInt(val) : 0;
                }} else {{
                    answers[q.id] = document.getElementById(q.id).value;
                }}
            }});

            // Switch to Loading view with steps animation
            document.getElementById('view-form').style.display = 'none';
            document.getElementById('view-processing').style.display = 'block';

            const steps = sharedUi.processing_steps || [
                "Reviewing the information provided",
                "Identifying the workflow friction",
                "Selecting a useful measurement",
                "Checking the response for safety and quality"
            ];
            
            let stepIdx = 0;
            const stepLbl = document.getElementById('lbl-processing-step');
            const interval = setInterval(() => {{
                if (stepIdx < steps.length - 1) {{
                    stepIdx++;
                    stepLbl.textContent = steps[stepIdx];
                }}
            }}, 600);
            ...
            try {{
                const response = await fetch('/api/analyze', {{
                    ...
                    body: JSON.stringify({{ scenario_id: scenarioId, answers: answers }})
                }});
                ...
                summary.innerHTML = `
                    <div class="error-summary">
                        <div class="error-summary-title">Network / Service Error</div>
                        <p class="error-summary-text">${{escapeHtml(sharedUi.error_messages.api_failure)}}</p>
                    </div>
                `;
            }}
        }}

        function renderBriefResult(brief, scenarioConfig) {{
            const container = document.getElementById('result-brief-container');
            container.innerHTML = '';

            const status = brief.brief_status;

            if (status === 'REVISE' || status === 'BLOCKED') {{
                let safeMessage = sharedUi.error_messages.blocked_request;
                if (status === 'REVISE') {{
                    safeMessage = "Scenario Brief was not approved or requires revision. Details withheld.";
                }}

                let revInstructionsHtml = '';
                if (status === 'REVISE' && brief.revision_instructions) {{
                    revInstructionsHtml = `
                        <div class="withheld-instructions">
                            <strong>Revision instructions:</strong> ${{escapeHtml(brief.revision_instructions)}}
                        </div>
                    `;
                }}
                ...
                container.innerHTML = `
                    <div class="withheld-card">
                        <div class="withheld-title">${{escapeHtml(status)}}</div>
                        <p class="withheld-message">${{escapeHtml(safeMessage)}}</p>
                        ${{revInstructionsHtml}}
                    </div>
                `;
            }} else {{
                // APPROVED or APPROVED_WITH_LIMITATION
                let bannerHtml = '';
                if (status === 'APPROVED_WITH_LIMITATION') {{
                    bannerHtml = `
                        <div class="limitation-banner">
                            <strong>Use with caution:</strong> ${{escapeHtml(config.required_disclosures.approved_with_limitation_banner)}}
                        </div>
                    `;
                }}
                ...
                // Redaction banner
                let redactionHtml = '';
                if (brief.redaction?.applied) {{
                    const categories = brief.redaction.categories.join(', ');
                    redactionHtml = `
                        <div class="limitation-banner" style="background-color: rgba(239, 68, 68, 0.05); border-color: rgba(239, 68, 68, 0.2); color: #fca5a5; margin-bottom: 24px;">
                            <strong>Privacy Redaction Applied:</strong> Sanitize triggered for categories: [${{escapeHtml(categories)}}].
                        </div>
                    `;
                }}

                // Assumptions list
                let assumptionsListHtml = '';
                if (brief.assumptions && brief.assumptions.length > 0) {{
                    const listItems = brief.assumptions.map(a => `<li>${{escapeHtml(a)}}</li>`).join('');
                    assumptionsListHtml = `<ul class="brief-bullets">${{listItems}}</ul>`;
                }} else {{
                    assumptionsListHtml = '<p>None.</p>';
                }}

                // Unknowns list
                let unknownsListHtml = '';
                if (brief.unknowns && brief.unknowns.length > 0) {{
                    const listItems = brief.unknowns.map(u => `<li>${{escapeHtml(u)}}</li>`).join('');
                    unknownsListHtml = `<ul class="brief-bullets">${{listItems}}</ul>`;
                }} else {{
                    unknownsListHtml = '<p>None.</p>';
                }}
                ...
                // CTA
                const cta = brief.episode_cta || {{}};

                container.innerHTML = `
                    <div class="brief-card">
                        <div class="brief-header">
                            <div class="brief-title">${{escapeHtml(brief.scenario_title)}}</div>
                            <span class="badge" style="${{status === 'APPROVED' ? 'background-color: rgba(16, 185, 129, 0.15); color: #34d399;' : 'background-color: rgba(245, 158, 11, 0.15); color: #fbbf24;'}}">${{escapeHtml(status)}}</span>
                        </div>
                        ...
                        <div class="brief-section">
                            ...
                            <div class="brief-text">${{escapeHtml(brief.what_we_heard)}}</div>
                        </div>

                        <div class="brief-section">
                            <div class="brief-section-title">Where Friction May Be Occurring</div>
                            <div class="brief-text">${{escapeHtml(brief.where_friction_may_be_occurring)}}</div>
                        </div>
                        ...
                        <div class="brief-section">
                            <div class="brief-section-title">One Next Step</div>
                            <div class="brief-text"><strong>${{escapeHtml(brief.one_next_step)}}</strong></div>
                        </div>

                        <div class="brief-section">
                            <div class="brief-section-title">Why This Step</div>
                            <div class="brief-text">${{escapeHtml(brief.why_this_step)}}</div>
                        </div>

                        <div class="brief-section">
                            <div class="brief-section-title">Recommended Measurement</div>
                            <div class="metrics-grid">
                                <div class="metric-card">
                                    <span class="metric-label">Observation Metric</span>
                                    <span class="metric-value" style="font-size: 1.15rem;">${{escapeHtml(m.name)}}</span>
                                </div>
                                <div class="metric-card">
                                    <span class="metric-label">Baseline Value</span>
                                    <span class="metric-value">${{escapeHtml(m.baseline_display)}}</span>
                                    <span class="metric-sub">Strength: ${{escapeHtml(m.evidence_strength)}}</span>
                                </div>
                            </div>
                            <div style="margin-top: 16px; font-size: 0.95rem; color: var(--text-secondary);">
                                <strong>Measurement Period:</strong> ${{escapeHtml(m.period)}}<br>
                                <strong>How to Track:</strong> ${{escapeHtml(m.calculation_method)}}
                            </div>
                        </div>
                        ...
                        <div class="disclosure-box">
                            <div class="disclosure-text">
                                <strong>Transparency Notice:</strong> ${{escapeHtml(brief.human_review_reminder)}}
                            </div>
                            <div class="disclosure-text">
                                <strong>Responsible AI disclaimer:</strong> ${{escapeHtml(brief.responsible_use_limitation)}}
                            </div>
                            <div class="cta-card">
                                <span class="cta-title">${{escapeHtml(cta.title)}}</span>
                                <span class="cta-desc">${{escapeHtml(cta.description)}}</span>
                                <a href="${{escapeHtml(cta.url)}}" target="_blank" class="cta-link">
                                    Listen to this podcast episode &rarr;
                                </a>
                            </div>
                        </div>
                    </div>
...
def get_public_episode_01_config() -> dict:
    """Legacy backward-compatible config loader for tests/older routes."""
    config = get_public_scenario_lab_config()
    cool_down_tax = config["scenarios"].get("cool_down_tax", {})
    return {
        "app": config["app"],
        "shared_ui": config["shared_ui"],
...
def render_episode_01_page(config: dict, values: dict | None = None, errors: dict | None = None) -> str:
    """Renders the complete Episode 01 questionnaire HTML page using config settings (legacy fallback)."""
    app_meta = config.get("app", {})
    shared_ui = config.get("shared_ui", {})
    scenario = config.get("scenario") or config.get("scenarios", {}).get("cool_down_tax", {})
